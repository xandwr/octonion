...
mod tests {
    ...
    fn basis_squares_are_minus_one() {
        ...
        for e in [
            ...
            assert_eq!(e * e, minus_one, "e² should be -1");
        }
    }
    ...
    fn half_integer_addition() {
        ...
        // Sum of two half-integers with same pattern → integer
        let sum = h1 + h2;
        assert!(
            sum.is_integer(),
    ...
    #[test]
    fn invalid_e8_point_rejected() {
        // This should fail: mixed parities
        let result = std::panic::catch_unwind(|| IntegralOctonion::new(1, 2, 0, 0, 0, 0, 0, 0));
        assert!(result.is_err(), "mixed parity should panic");

        // This should fail: all odd but sum not ≡ 0 (mod 4)
        // sum = 1+1+1+1+1+1+1+3 = 10 ≡ 2 (mod 4)
        let result = std::panic::catch_unwind(|| IntegralOctonion::new(1, 1, 1, 1, 1, 1, 1, 3));
        assert!(result.is_err(), "invalid sum should panic");
    }
